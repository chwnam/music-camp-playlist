 ...
 *
 * etc/class-mcpl-ejs-queue.php
 */
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

if ( ! class_exists( 'MCPL_EJS_Queue' ) ) {
	class MCPL_EJS_Queue implements MCPL_Module {
		...
		public function __construct() {
			if ( is_admin() ) {
				if ( ! has_action( 'admin_print_footer_scripts', [ $this, 'do_template' ] ) ) {
					add_action( 'admin_print_footer_scripts', [ $this, 'do_template' ], mcpl_priority() );
				}
			} elseif ( ! has_action( 'wp_print_footer_scripts', [ $this, 'do_template' ] ) ) {
				add_action( 'wp_print_footer_scripts', [ $this, 'do_template' ], mcpl_priority() );
			}
		}
		 ...
		 *
		 * @return void
		 * @see    MCPL_Template_Impl::enqueue_ejs()
		 */
