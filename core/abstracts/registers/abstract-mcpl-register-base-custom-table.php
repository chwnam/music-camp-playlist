 ...
 *
 * abstracts/registers/abstract-mcpl-register-base-custom-table.php
 */
...
if ( ! class_exists( 'MCPL_Register_Base_Custom_Table' ) ) {
	abstract class MCPL_Register_Base_Custom_Table implements MCPL_Register {
		...
		public function register(): void {
			...
			foreach ( $this->get_items() as $item ) {
				if ( $item instanceof MCPL_Reg_Custom_Table ) {
		...
		public function initial_setup(): void {
			...
			$install_initial_data = apply_filters( 'mcpl_install_initial_data', true, $this->get_current_version(), $this->get_installed_version() );
		...
		public function update_table(): void {
			...
			if ( $new_version !== $old_version ) {
				do_action( 'mcpl_before_update_table', $new_version, $old_version );
				$this->register();
				do_action( 'mcpl_after_update_table', $new_version, $old_version, $this->delta_result );
				$this->update_version( $this->get_current_version() );
				$this->log_delta_result();
			}
		}
		...
		public function get_installed_version(): string {
			return get_option( 'mcpl_db_version', '' );
		}
		...
		public function update_version( string $version ): void {
			update_option( 'mcpl_db_version', $version );
		}
		...
		protected function log_delta_result(): void {
			if ( $this->delta_result ) {
				...
				error_log( "dbDelta() updated 'mcpl_db_version' to $version.\n" . $this->delta_result );
			}
		}
	}
}
