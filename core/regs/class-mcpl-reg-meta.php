...
if ( ! class_exists( 'MCPL_Reg_Meta' ) ) {
	 ...
	 */
	class MCPL_Reg_Meta implements MCPL_Reg {
			...
			if ( isset( $wp_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
				...
				if ( ! isset( static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
					static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] =
						new MCPL_Reg_Meta( $object_type, $meta_key, $args );
				}
			...
			if ( $this->object_type && $this->get_key() ) {
				try {
					if ( $this->args['sanitize_callback'] ) {
						$this->args['sanitize_callback'] = MCPL_Main::get_instance()->parse_callback( $this->args['sanitize_callback'] );
					}
					...
					$error->add(
						...
						sprintf(
							...
							mcpl_format_callable( $this->args['sanitize_callback'] )
						)
					);
				...
				try {
					if ( $this->args['auth_callback'] ) {
						$this->args['auth_callback'] = MCPL_Main::get_instance()->parse_callback( $this->args['auth_callback'] );
					}
				} catch ( MCPL_Callback_Exception $e ) {
					$error = new WP_Error();
					$error->add(
						...
						sprintf(
							...
							mcpl_format_callable( $this->args['auth_callback'] )
						)
					);
					// $error is a WP_Error instance.
					// phpcs:ignore WordPress.Security.EscapeOutput
