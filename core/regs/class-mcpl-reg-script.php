 ...
 *
 * regs/class-mcpl-reg-script.php
 */
...
if ( ! class_exists( 'MCPL_Reg_Script' ) ) {
	class MCPL_Reg_Script implements MCPL_Reg {
		public const WP_SCRIPT = 'mcpl-wp-script-generated';
			...
			if ( $this->handle && $this->src && ! wp_script_is( $this->handle, 'registered' ) ) {
				if ( self::WP_SCRIPT === $this->deps ) {
					...
					if ( $is_theme ) {
						...
						$root = plugin_dir_url( mcpl_main_file() ) . 'assets/js/';
					}
					...
					if ( $is_theme ) {
						...
						$path = path_join( dirname( mcpl_main_file() ), "assets/js/$dir/$file" );
					}
					...
					if ( $is_theme ) {
						...
						$this->src = plugins_url( "assets/js/$this->src", mcpl_main_file() );
					}

					$this->deps      = $info['dependencies'] ?? [];
					$this->ver       = $info['version'] ?? mcpl_version();
					$this->in_footer = true;
				}
		 ...
		 *
		 * @return MCPL_Reg_Script
		 */
		public static function wp_script( string $handle, string $src ): MCPL_Reg_Script {
			return new MCPL_Reg_Script( $handle, $src, self::WP_SCRIPT );
		}
	}
}
