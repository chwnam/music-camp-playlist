 ...
 *
 * traits/trait-mcpl-hook-impl.php
 */
...
if ( ! trait_exists( 'MCPL_Hook_Impl' ) ) {
	trait MCPL_Hook_Impl {
		/**
		 * Modified add_action().
		 ...
		 * @param int|null                  $priority         If null, priority comes from 'MCPL_PRIORITY' constant.
		...
		private function hook_get_priority( ?int $priority ): int {
			return is_null( $priority ) ? mcpl_priority() : $priority;
		}
	}
}
