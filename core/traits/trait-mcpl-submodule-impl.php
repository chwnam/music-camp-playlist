 ...
 *
 * traits/trait-mcpl-submodule-impl.php
 */
...
if ( ! trait_exists( 'MCPL_Submodule_Impl' ) ) {
	trait MCPL_Submodule_Impl {
		/**
		 * Submodules
		 *
		 * @var array
		...
		protected function assign_modules( array $modules, bool $auto_wrap = false ): self {
			$constructor_params = MCPL_Main_Base::get_instance()->get_constructor_params();
		...
		protected function invoke_module( string $name, Closure $module ) {
			$constructor_paramss = MCPL_Main_Base::get_instance()->get_constructor_params();
