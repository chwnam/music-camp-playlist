 ...
 *
 * traits/trait-mcpl-autobind-impl.php
 */
...
if ( ! trait_exists( 'MCPL_Autobind_Impl' ) ) {
	trait MCPL_Autobind_Impl {
		/**
		 ...
		 *
		 * Rule:
		 * - {mcpl}:{module_notation}@{callback}?foo&bar
		 * - {mcpl}:{module_notation}@{callback}!
		 *
		 * NONCE:
		...
		public function parse_autobind(): array|false {
			...
			$regex  = '/^mcpl:(([A-Za-z0-9_.]+)@)?([A-Za-z0-9_]+)(!|\?([A-Za-z0-9\-_&]*))?(\*?)$/';
			...
			try {
				$module   = $matches[2] ?? '';
				$func     = $matches[3] ?? '';
				$callback = mcpl_parse_callback( $module ? "$module@$func" : $func );
			} catch ( MCPL_Callback_Exception $e ) {
				wp_die( $e->getMessage() );
			}

			$nonce_action = '_mcpl_nonce';
