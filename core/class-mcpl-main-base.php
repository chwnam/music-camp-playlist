...
if ( ! class_exists( 'MCPL_Main_Base' ) ) {
	/**
	 * Class MCPL_Main_Base
	 */
	class MCPL_Main_Base implements MCPL_Module {
		use MCPL_Hook_Impl;
		 ...
		 * Singleton instance
		 */
		protected static ?MCPL_Main_Base $instance = null;
		...
		public static function set_instance( MCPL_Main_Base $instance ): void {
			static::$instance = $instance;
			static::$instance->initialize();
		}

		/**
		 * MCPL_Main_Base constructor.
		 */
		...
		public function __sleep() {
			throw new Exception( 'MCPL_Main does not support object serialization.' );
		}
		...
		public function parse_callback( Closure|array|string $item ): Closure|array|string {
			...
			throw new MCPL_Callback_Exception(
				sprintf(
				/* translators: formatted module name. */
					__( '%s is invalid for callback.', 'mcpl' ),
					mcpl_format_callable( $item )
				)
			);
		}
		...
		public function load_textdomain(): void {
			if ( mcpl_is_theme() ) {
				...
				load_plugin_textdomain(
					'mcpl',
					false,
					wp_basename( dirname( mcpl_main_file() ) ) . '/languages'
				);
			}
		}
		...
		public function activation(): void {
			do_action( 'mcpl_activation' );
		}

		public function deactivation(): void {
			do_action( 'mcpl_deactivation' );
		}
		 ...
		 *
		 * @uses MCPL_Main_Base::init_conditional_modules()
		 * @uses MCPL_Main_Base::load_textdomain()
		 */
		protected function initialize(): void {
			$this
				 ...
				 */
				->add_action( 'init', 'assign_later_modules', mcpl_priority() + 10 )
			;

			if ( mcpl_is_theme() ) {
				$this->add_action( 'after_setup_theme', 'load_textdomain', mcpl_priority() + 10 );
			} else {
				$this->add_action( 'plugins_loaded', 'load_textdomain', mcpl_priority() + 10 );
			}
		...
		protected function setup_activation_deactivation(): self {
			if ( mcpl_is_theme() ) {
				...
				$file = plugin_basename( mcpl_main_file() );
				$this
		 ...
		 * @return $this
		 */
		protected function assign_constructors( array $constructors ): MCPL_Main_Base {
		...
		/**
		 * Do MCPL_Main specific initialization.
		 */
