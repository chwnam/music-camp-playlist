...
if ( ! class_exists( 'MCPL_Register_Theme_Support' ) ) {
	class MCPL_Register_Theme_Support extends MCPL_Register_Base_Theme_Support {
		...
		public function get_items(): Generator {
			yield new MCPL_Reg_Theme_Support( 'automatic-feed-links' );

			yield new MCPL_Reg_Theme_Support( 'post-thumbnails' );

			yield new MCPL_Reg_Theme_Support(
				'post-formatst',
				[
					'aside',
					'gallery',
			...
			yield new MCPL_Reg_Theme_Support( 'title-tag' );
		}
		...
		public function map_front_modules( WP_Query $query ) {
			...
			*/
			...
			if ( $hierarchy->is_archive() ) {
				...
				if ( $module instanceof MCPL_Front_Archive_Module ) {
		...
		protected function extra_register(): void {
			...
			register_nav_menus(
				[
					'primary'   => __( 'Primary Menu', 'mcpl' ),
					'secondary' => __( 'Secondary Menu', 'mcpl' ),
				]
			);

			register_sidebar(
				[
					'name'          => __( 'Primary Sidebar', 'mcpl' ),
					'id'            => 'sidebar-1',
			...
			register_sidebar(
				[
					'name'          => __( 'Secondary Sidebar', 'mcpl' ),
					'id'            => 'sidebar-2',
			...
			/** @see MCPL_Main::load_textdomain() for textdomain loading. */
		}
	}
}
