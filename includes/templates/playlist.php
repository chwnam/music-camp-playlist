...
<?php if ( $items ): ?>
	<div class="row">
		<div id="calendar" class="col-9 col-sm-5 col-md-4 col-lg-3 pt-2 mx-auto"></div>
		<div id="playlist" class="col-12 col-sm-7 col-md-8 col-lg-9">
			<?php foreach ( $items as $date => $daily_playlist ) : ?>
				<?php $d = date_create_immutable( $date ); ?>
				<h5 class="mt-3 mb-2 d-flex justify-content-between align-items-baseline">
					<?php echo esc_html( $d ? $d->format( 'Y년 m월 d일' ) : '' ); ?>
					<div id="controls" class="float-end">
						<a id="check-playlist"
						   href="<?php echo esc_url( $page_url ); ?>"
						   class="text-decoration-none">편성표 확인</a>
						<span class="text-black-50">|</span>
						<a id="refetch-playlist"
						   href="#"
						   class="text-decoration-none">다시 수집</a>
					</div>
				</h5>
				<ul class="list-unstyled mb-2">
					<?php foreach ( $daily_playlist as $idx => $item ): ?>
						<li>
							<div class="row m-0">
								<div
									class="col my-1 py-2 border border-secondary-subtle border-top-0 border-start-0 border-end-0">
									<div class="row">
				...
				<div class="col d-flex justify-content-between mb-5 px-4">
					<?php if ( $next ) : ?>
						<a id="prev"
						   href="<?php echo esc_url( add_query_arg( 'date', $next ) ); ?>"
						   class="text-decoration-none">다음 날짜</a>
					<?php else: ?>
						<span>&nbsp;</span>
					<?php endif; ?>
					...
					<?php if ( $prev ) : ?>
						<a id="next"
						   href="<?php echo esc_url( add_query_arg( 'date', $prev ) ); ?>"
						   class="text-decoration-none">이전 날짜</a>
					<?php else: ?>
						<span>&nbsp;</span>
					<?php endif; ?>
				</div>
