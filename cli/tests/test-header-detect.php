...
class Test_Header_Detect extends TestCase {
	...
	public function test_detect( array|false $expected, string $root_dir ) {
		$actual = MCPL_CLI_Header_Detect::detect( $root_dir );
		...
		return [
			[
				[
					...
					'type'    => MCPL_CLI_Header_Detect::TYPE_PLUGIN,
					'version' => '0.1.0',
				],
			...
			[
				[
					...
					'type'    => MCPL_CLI_Header_Detect::TYPE_THEME,
					'version' => '1.1.0',
				],
	...
	public function test_get_header_fields( array $expected, string $file, array $fields ) {
		$actual = MCPL_CLI_Header_Detect::get_header_fields( $file, $fields );
		$this->assertEquals( $expected, $actual );
	}
	...
	public function test_extract_header( string $content, array $cases ) {
		foreach ( $cases as $case ) {
			$actual = MCPL_CLI_Header_Detect::extract_header( $content, $case['header'] );
