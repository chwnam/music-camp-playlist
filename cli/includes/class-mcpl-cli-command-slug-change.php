<?php
if ( ! class_exists( 'MCPL_CLI_Command_Slug_Change' ) ) {
	final class MCPL_CLI_Command_Slug_Change implements MCPL_CLI_Command {
		private MCPL_CLI_Regex_Replace $regex_replace;

		private MCPL_CLI_Textdomain_Replace $textdomain_replace;
		...
		private string $root_dir = MCPL_ROOT;
		...
		public function initialize( Console_CommandLine_Result $parsed ): void {
			do {
				...
				if ( mcpl_cli_the_slug() !== $this->old_slug ) {
					self::validate_slug( $this->old_slug );
				}
				self::validate_slug( $this->new_slug );

				if ( mcpl_cli_the_slug() !== $this->old_textdomain ) {
			...
			$this->regex_replace      = new MCPL_CLI_Regex_Replace( $this->old_slug, $this->new_slug );
			$this->textdomain_replace = new MCPL_CLI_Textdomain_Replace( $this->old_textdomain, $this->new_textdomain );
		}
		...
		private function slug_prompt(): string {
			do {
				$confirmed = false;
				$new_slug  = mcpl_cli_prompt( 'Please enter your new slug (Enter \'exit\' to skip):' );
				if ( 'exit' === $new_slug ) {
