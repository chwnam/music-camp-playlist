...
if ( ! class_exists( 'MCPL_CLI_Node_Visitor_Node' ) ) {
	/**
	 * Visit node and keep track of what we need
	 */
	class MCPL_CLI_Node_Visitor_Node extends NodeVisitorAbstract implements MCPL_CLI_Node_Visitor {
		...
		private function extract_from_node( Node $node ): void {
			if ( $node instanceof Identifier ) {
				...
				if ( $node instanceof VarLikeIdentifier ) {
					...
					$this->add_token(
						MCPL_CLI_Token::TYPE_IDENTIFIER,
						$node->name,
				...
				if ( $position && isset( $node->args[ $position ] ) ) {
					...
					if ( $textdomain instanceof ScalarString ) {
						$this->add_token(
							MCPL_CLI_Token::TYPE_TEXTDOMAIN,
							$textdomain->value,
		...
		private function add_token(
			...
			if ( $overwrite || ! isset( $this->tokens[ $startPos ] ) ) {
				$this->tokens[ $startPos ] = new MCPL_CLI_Token( $type, $value, $startPos, $endPos );
			}
		}
	}
}
