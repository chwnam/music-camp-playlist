<?php
if ( ! class_exists( 'MCPL_CLI_App' ) ) {
	final class MCPL_CLI_App {
		public function run(): void {
			...
			foreach ( $this->get_commands() as $command ) {
				if ( $parsed->command_name === call_user_func( [ $command, 'get_command_name' ] ) ) {
					...
					if ( $instance instanceof MCPL_CLI_Command ) {
		...
		private function build_parser(): Console_CommandLine {
			...
			foreach ( $this->get_commands() as $command ) {
				...
				if ( isset( $implements[ MCPL_CLI_Command::class ] ) ) {
		...
		private function get_commands(): array {
			return [
				MCPL_CLI_Command_Make_Zip::class,
				MCPL_CLI_Command_Remove_Hot_Update::class,
				MCPL_CLI_Command_Slug_Change::class,
				MCPL_CLI_Command_Sync_Version::class,
			];
		}
