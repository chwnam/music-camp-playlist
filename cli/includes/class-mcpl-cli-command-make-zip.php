<?php
if ( ! class_exists( 'MCPL_CLI_Command_Make_Zip' ) ) {
	final class MCPL_CLI_Command_Make_Zip implements MCPL_CLI_Command {
		...
		public function run( Console_CommandLine_Result $parsed ): void {
			...
			if ( empty( $output ) ) {
				$output = dirname( $project_dir ) . '/' . mcpl_cli_basename( $project_dir ) . '.zip';
			}
			...
			$zip->addEmptyDir( mcpl_cli_basename( $project_dir ) );
