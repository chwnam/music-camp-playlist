...
if ( ! class_exists( 'MCPL_CLI_Command_Sync_Version' ) ) {
	final class MCPL_CLI_Command_Sync_Version implements MCPL_CLI_Command {
		...
		public function run( Console_CommandLine_Result $parsed ): void {
			...
			if ( $parsed->options['main_file'] ) {
				...
				$result    = MCPL_CLI_Header_Detect::get_header_data( $main_file );
			} else {
				// Discover.
				$result    = MCPL_CLI_Header_Detect::detect( MCPL_ROOT );
			...
			if ( MCPL_CLI_Header_Detect::TYPE_THEME === $result['type'] && $main_file ) {
			...
			$slug_data = mcpl_cli_get_dot_slug();
			$slug      = $slug_data['slug'] ?? mcpl_cli_the_slug();
			...
			echo '* Main file: ' . substr( $main_file, strlen( MCPL_ROOT ) + 1 ) . PHP_EOL;
			...
			$files = [
				$main_file,
				$functions_file,
				MCPL_ROOT . '/composer.json',
				MCPL_ROOT . '/package.json',
			];
		...
		public function apply_change_php(
			...
			$extractor = new MCPL_CLI_Token_Extract( new MCPL_CLI_Node_Visitor_Const() );
			...
			if ( $content ) {
				 ...
				 *
				 * @var MCPL_CLI_Token_Const[] $tokens
				 */
		...
		public function apply_change_json( string $json_path, string $target_version ): void {
			...
			if ( is_array( $content ) ) {
				...
				if ( $version !== $target_version ) {
					$file_name = substr( $json_path, strlen( MCPL_ROOT ) + 1 );
		...
		private function replace_header_version( string $content, string $slug, string $target_version ): string {
			if ( $content ) {
				...
				[ $value, $offset ] = MCPL_CLI_Header_Detect::extract_header( $content, 'Version' );
				...
				[ $value, $offset ] = MCPL_CLI_Header_Detect::extract_header( $content, 'CPBN Version' );
				if ( mcpl_cli_the_slug() === $slug && $offset && $value !== $target_version ) {
